            ...
            <div class="cards_layouts hap_id_number_layouts">
              <div class="hap_id_number_address">
                ...
                <div class="results_address hap_number_address_title">
                  <h3>{{ hapIdData.name }}</h3>
                  <li>
                    <i class="fas fa-map-marker-alt"></i>
                    {{
                      `${hapIdData.address}, ${hapIdData.zip_code}, ${
                        hapIdData.country && hapIdData.country.name
                      }, ${hapIdData.state && hapIdData.state.name}`
                    }}
                  </li>
                  <li>
  ...
  methods: {
    ...
    async onSearchHandler() {
      const result = await ApiClass.getRequest(
        `company_hap_id/${this.search}/${true}`,
        this.isLoggedIn
      );
      if (result.data.status_code === "1") {
        this.search = "";
